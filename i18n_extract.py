...
class I18nExtract:
    regex = r"polyglot.t\(['\"](.*)['\"]\)"
    __correct_index = []
    __file_extention = "*.jade"

    def __init__(self, template_folder, locale_folder, file_extention):
    ...
    def __extract_template_keys(self):
        ...
        template = [
            ...
            for f in fnmatch.filter(files, self.__file_extention)
        ]
        ...
        for file_ in template:
            ...
            print "%s index were found in %s: %s\n" % (len(results), file_, results)
...
try:
    if len(sys.argv) > 1:
        ...
        if templates_folder is not None and locales_folder is not None:
            I18nExtract(templates_folder, locales_folder, file_extention)
    else:
